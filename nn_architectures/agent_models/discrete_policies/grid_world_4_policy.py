...
class GRID_WORLD_4_POLICY(torch.nn.Module):
    """
    ...
    def __init__(self, state_size, action_size, actions, hidden_layer_size):
        super(GRID_WORLD_4_POLICY, self).__init__()
        ...
        self.linear3 = torch.nn.Linear(hidden_layer_size, hidden_layer_size)
        self.linear4 = torch.nn.Linear(hidden_layer_size, actions)
        # output
    ...
    def sample_action(self, state):
        ...
        # add non zero prob of other actions
    ...
    def forward(self, state, action):
        ...
        action = action.long()
        probabilities = self.linear1(input)
        probabilities = F.tanh(probabilities)
        probabilities = self.linear2(probabilities)
        probabilities = torch.tanh(probabilities)
        probabilities = self.linear3(probabilities)
        probabilities = torch.tanh(probabilities)
