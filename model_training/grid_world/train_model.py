...
class TRAIN_AGENT_GRID_WORLD(torch.nn.Module):
    ...
    def init_enviroment(self):
        ...
        # initialize the grid
        grid = reward_surface.generate_reward_surface('IL_1')
        # set start state
    ...
    def train_agent(self):
        ...
        self.enviroment = self.init_enviroment()
        ...
        for iter in range(self.iterations):
            ...
            # get new batch of trajectories from simulator (sample with old policy)
            state_total, action_total, reward_total = self.enviroment.simulate_trajectory(self.trajectory_length, self.sample_size, policy)

            """ RUN MULTIPLE UPDATES ON GPU USING MINI-BATCHES """
            # initialize pytorch dataset for efficient data loading
            data = TensorDataset(state_total, action_total.long(), reward_total)
            # set data loader
            data_loader = DataLoader(data, batch_size=self.batch_size, shuffle=True, num_workers=self.workers)
            # set average to so current expectation
            var = None
            # update model on gpu following mini-batches
            for r, (state_batch, action_batch, reward_batch) in enumerate(data_loader):
        ...
        return policy, loss_per_iteration
